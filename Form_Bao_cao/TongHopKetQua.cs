...
{
    public partial class TongHopKetQua : Form
    {
        ...
        private void TongHopKetQua_Load(object sender, EventArgs e)
        {
        ...
        private void btnXuatBaoCao_Click(object sender, EventArgs e)
        {
            ...
            Aspose.Words.Document baoCao = new Aspose.Words.Document("Template\\Bao_Cao_TH_Template.doc");
            ...
            tableBaoCao.InsertRows(hangHienTai, hangHienTai, soDong - 1);

            loadData();

            for(int i = 0; i < soDong; i++)
            {
                tableBaoCao.PutValue(i + 1, 0, (i+1).ToString());
                tableBaoCao.PutValue(i + 1, 1, dgv.Rows[i].Cells[3].Value.ToString());
                tableBaoCao.PutValue(i + 1, 2, dgv.Rows[i].Cells[2].Value.ToString());
                tableBaoCao.PutValue(i + 1, 3, dgv.Rows[i].Cells[4].Value.ToString());
                tableBaoCao.PutValue(i + 1, 4, dgv.Rows[i].Cells[5].Value.ToString());
                tableBaoCao.PutValue(i + 1, 5, dgv.Rows[i].Cells[8].Value.ToString());
            }
            
            //Lưu và mở file
            baoCao.SaveAndOpenFile("Bao_Cao_Tong_Hop.docx");
